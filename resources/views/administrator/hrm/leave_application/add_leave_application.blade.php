    ...
    <div class="box box-default bt-none">
      ...
      <form action="{{ url('/hrm/leave_application/store') }}" method="post" name="add_form_leave_application">
            ...
            <div class="col-md-6">
              ...
              <div class="form-group{{ $errors->has('last_leave_period') ? ' has-error' : '' }} has-feedback">
                <input type="text" name="last_leave_period" id="last_leave_period" class="form-control"
                  value="{{ old('last_leave_period') }}" placeholder="{{ __('Enter Period of Last Leave..') }}">
              ...
              <div class="form-group{{ $errors->has('leave_address') ? ' has-error' : '' }} has-feedback">
                <textarea class="textarea text-description" name="leave_address"
                  placeholder="{{ __('Enter leave_address..') }}">{{ old('leave_address') }}</textarea>
              ...
              <label for="during_leave">{{ __('Performing person during leave') }} <span
              ...
              <div class="form-group{{ $errors->has('during_leave') ? ' has-error' : '' }} has-feedback">
                <input type="text" name="during_leave" id="during_leave" class="form-control"
                  value="{{ old('during_leave') }}" placeholder="{{ __('Enter Performing person during leave..') }}">
            ...
            <div class="col-md-12">
              ...
              <div class="form-group{{ $errors->has('reason') ? ' has-error' : '' }} has-feedback">
                <textarea class="textarea text-description" name="reason"
                  placeholder="{{ __('Enter reason Details..') }}">{{ old('reason') }}</textarea>
        ...
        <div class="box-footer">
          <a href="{{ url('/hrm/leave_application') }}" class="btn btn-danger btn-flat"><i class="icon fa fa-close"></i>
            {{ __('Cancel') }}</a>
          <button type="submit" class="btn btn-primary btn-flat"><i class="icon fa fa-plus"></i>
            {{ __('Add leave application') }}</button>
        </div>
      </form>
    </div>
    <!-- /.box -->
