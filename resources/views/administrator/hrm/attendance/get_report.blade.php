                                ...
                                <div class="input-group margin">
                                    ...
                                    <span class="input-group-btn">
                                        <button type="submit" class="btn btn-info btn-flat"><i
                                                class="icon fa fa-arrow-right"></i> {{ __('Go') }}</button>
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <table class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                ...
                                @for($i=1; $i<=$number_of_days; $i++) <th class="text-center">{{ $i }}</th>
                                    @endfor
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                <td><a
                                        href="{{ url('/people/employees/details/' . $employee['id']) }}">{{ $employee['name'] }}</a>
                                </td>
                                @for($i = 1; $i <= $number_of_days; $i++) <td class="text-center">
                                    @if($i>=1 AND $i<=9) @php($day=$date."-0".$i) @else @php($day=$date."-".$i) @endif
                                        @php($day_name=date("D", strtotime($day))) @foreach($monthly_holidays as
                                        $monthly_holiday) @if($day==$monthly_holiday['date']) <span
                                        class="btn btn-xs btn-danger btn-flat" data-toggle="tooltip"
                                        data-original-title="{{ $monthly_holiday['holiday_name'] }}">
                                        {{ __('H') }}</span>
                                        @endif
                                        @endforeach
                                        ...
                                        <span class="btn btn-xs btn-danger btn-flat" data-toggle="tooltip"
                                            data-original-title="Weekly Holiday">{{ __('H') }}</span>
                                        @endif
                                        @endforeach
                                        ...
                                        <span class="btn btn-xs btn-success btn-flat" data-toggle="tooltip"
                                            data-original-title="Present"> {{ __('P') }} </span>
                                        @endif
                                        ...
                                        <span class="btn btn-xs btn-warning btn-flat" data-toggle="tooltip"
                                            data-original-title="Absence">{{ __('A') }}</span>
                                        @else
                                        <span class="btn btn-xs btn-info btn-flat" data-toggle="tooltip"
                                            data-original-title="{{ $attendance['leave_category'] }}">{{ __('L') }}</span>
                                        @endif
