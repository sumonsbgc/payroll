        ...
        <div class="box bt-none">
            ...
            <div class="box-body">
                <div class=" text-right">
                    ...
                    <a href="{{ url('hrm/salary/statement/search') }}" class="py-1 px-5 btn bg-green text-uppercase"><i
                            class="fa fa-plus"></i>
                        {{ __('add new Salary Statement') }}</a>

                    <form action="{{ url('/hrm/salary/statement/preview') }}" method="get" class="d-inline-block">
                        {{ csrf_field() }}
                        ...
                        <button type="submit" class="btn bg-blue py-1 px-5 text-uppercase">{{ __('Preview') }}
                        </button>
                    </form>

                    <form action="{{ url('/hrm/salary/statement/pdf') }}" method="get" class="d-inline-block">
                        {{ csrf_field() }}
                        <input type="hidden" name="emp_id" value="{{$empid}}">
                        <input type="hidden" name="date1" value="{{$startdate}}">
                        <input type="hidden" name="date2" value="{{$enddate}}">
                        <button type="submit" class="btn bg-orange py-1 px-5">{{ __('PDF') }} </button>
                    </form>
                </div>
                <hr>
                ...
                <div class="st-center-body">
                    <div class="img-body"><img src="{{ asset('public/profile_picture/'.auth()->user()->avatar) }}"
                            class="img"></div>
