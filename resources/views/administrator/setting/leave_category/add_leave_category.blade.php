        ...
        <div class="box box-default bt-none">
            ...
            <form action="{{ url('setting/leave_categories/store') }}" method="post" name="leave_category_add_form">
                ...
                <div class="box-body">
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            ...
                            @else
                            <p class="text-yellow">{{ __('Enter leave_category details. All field are required.') }}
                            </p>
                            @endif
                        </div>
                        <!-- /.Notification Box -->
                        ...
                        <div class="col-md-6">
                            <label for="leave_category">{{ __('Category Name ') }}<span
                                    class="text-danger">*</span></label>
                            <div
                                class="form-group{{ $errors->has('leave_category') ? ' has-error' : '' }} has-feedback">
                                <input type="text" name="leave_category" id="leave_category" class="form-control"
                                    value="{{ old('leave_category') }}" placeholder="{{ __('Enter client name..') }}">
                        ...
                        <!-- /.col -->
                        <div class="col-md-12">
                            <label for="leave_category_description">{{ __('Category Description') }} <span
                                    class="text-danger">*</span></label>
                            <div
                                class="form-group{{ $errors->has('leave_category_description') ? ' has-error' : '' }} has-feedback">
                                <textarea class="textarea text-description" name="leave_category_description"
                                    id="leave_category_description"
                                    placeholder="{{ __('Enter client description..') }}">{{ old('leave_category_description') }}</textarea>
                                @if ($errors->has('leave_category'))
                                <span class="help-block">
                ...
                <div class="box-footer">
                    <a href="{{ url('/setting/leave_categories') }}" class="btn btn-danger btn-flat"><i
                            class="icon fa fa-close"></i>{{ __('Cancel') }} </a>
                    <button type="submit" class="btn btn-primary btn-flat"><i
                            class="icon fa fa-plus"></i>{{ __('Add leave category') }} </button>
                </div>
            </form>
        </div>
        <!-- /.box -->
